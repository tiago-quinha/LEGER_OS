...
export function UnnamedTransactionResolver({
  ...
  return (
    <>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] font-sans text-muted-foreground">
                  Latest: <span className="font-mono font-bold text-foreground">€{Math.abs(parseFloat(unnamedList[0].amount)).toFixed(2)}</span> ({unnamedList[0].merchant || "Card debit"}) · Tap to name store & auto-categorize
                </p>
              </div>
            </div>
      ...
      {/* Native Draggable Bottom Drawer Standard (Rule 14) */}
      <AnimatePresence>
        {activeTx && (
          <div className="fixed inset-0 z-[100] flex items-end justify-center">
            ...
            <motion.div
              ...
              className="relative w-full max-w-xl bg-[#09090b] border-t border-x border-border p-6 rounded-t-2xl shadow-2xl z-10 space-y-6 max-h-[90vh] overflow-y-auto"
            >
              {/* Top Drag Handle Indicator */}
              <div className="w-12 h-1 bg-muted-foreground/30 rounded-full mx-auto my-1 cursor-grab active:cursor-grabbing" />
              ...
              <div className="flex items-center justify-between border-b border-border/50 pb-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
                    ...
                    <span>Bank Push Capture // Name Merchant</span>
                  </div>
              ...
              <div className="p-4 bg-secondary/20 border border-border flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <div className="text-3xl font-mono font-bold tracking-tight text-foreground">
                    -€{Math.abs(parseFloat(activeTx.amount)).toFixed(2)}
                  </div>
                </div>
                <div className="text-right space-y-0.5 font-mono text-xs">
                  <span className="text-[9px] uppercase text-muted-foreground">Recorded Date</span>
                  <div className="text-foreground">
                    {new Date(activeTx.date).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })}
                  </div>
                </div>
              </div>
              ...
              {activeTx.raw_text && (
                <div className="text-[11px] font-mono text-muted-foreground/80 bg-secondary/10 p-2.5 border border-border/40 truncate">
                  <span className="text-muted-foreground uppercase text-[9px] block mb-0.5">Raw Bank Notification:</span>
                  {activeTx.raw_text}
                </div>
              )}
                    ...
                    {frequentMerchants.map((m) => (
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-mono border transition-all cursor-pointer ${
                          merchantInput.toLowerCase() === m.name.toLowerCase()
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    autoFocus
                    className="h-11 rounded-none font-mono text-sm bg-secondary/30 border-border focus-visible:ring-1 focus-visible:ring-foreground"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 max-h-36 overflow-y-auto p-1 border border-border/40 bg-secondary/10">
                  {categories.map((c) => {
              ...
              <div className="flex items-center gap-3 pt-2">
                <Button
                  ...
                  variant="outline"
                  onClick={closeResolver}
                  className="w-1/3 h-11 rounded-none border-border font-mono text-xs uppercase cursor-pointer"
                >
                  Cancel
                </Button>
                <Button
                  ...
                  onClick={handleSave}
                  className="w-2/3 h-11 rounded-none bg-foreground text-background hover:bg-foreground/90 font-mono text-xs uppercase font-bold tracking-wider cursor-pointer flex items-center justify-center gap-2"
                >
